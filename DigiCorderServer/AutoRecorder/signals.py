...
import http.client
from AutoRecorder.models import ActiveAircraft, NextTakeoffData, Airfield, AircraftType, Runway, AircraftType, Callsign, Trigger, ADSBSource
import copy
...
def log_completed_flight(sender, instance, created, **kwargs):
    try:    
        ...
        logger.error("Error in Log Completed Flight Function")
...
def nextTODisplayUpdate(sender, instance, created, **kwargs):
    try:
        ...
        logger.error("Error in nextTODisplayUpdate Function")
        logger.error(traceback.format_exc())
...
def displayActiveAircraft(sender, instance, created, **kwargs):
    try:
        ...
        logger.error("Error in displayActiveAircraft Function")
        logger.error(traceback.format_exc())
...
def createUserDisplayExtras(sender, instance, action, reverse, **kwargs):
    try:
        ...
        logger.error("Error in createUserDisplayExtras Function")
        logger.error(traceback.format_exc())
...
def createUserGroupForEachAirfield(sender, instance, created, **kwargs):
    try:
        ...
        logger.error("Error in createUserGroupForEachAirfield Function")
        logger.error(traceback.format_exc())
...
def createRunwayDependencies(sender, instance, created, **kwargs): 
    try:
        ...
        logger.error("Error in createRunwayDependencies Function")
        logger.error(traceback.format_exc())
...
def adsbCommsControl(sender, instance, created, **kwargs): 
    ...
    #do the same thing for the other two threads
        ...
        logger.error("Error in adsbCommsControl Function")
        logger.error(traceback.format_exc())
...
def get_ActiveAircraft_queryset():
    try:
        ...
        logger.error("Error in get_ActiveAircraft_queryset Function")
        logger.error(traceback.format_exc())
...
def get_Message_queryset():
    try:
        ...
        logger.error("Error in get_Message_queryset Function")
        logger.error(traceback.format_exc())
...
def get_next_takeoff_update_messages():
    try:
        ...
        logger.error("Error in get_next_takeoff_update_messages Function")
        logger.error(traceback.format_exc())
...
def distance(lat1, lon1, alt1, lat2, lon2, alt2): #Calculates the straing line distance beween 2 points in space. (Lat, Lon, Feet, Lat, Lon, Feet)
    try:
        ...
        logger.error("Error in distance Function")
        logger.error(traceback.format_exc())
...
def messageThread(freq, parentThreadName):
    try:
        ...
        logger.error("Error in messageThread Function")
        logger.error(traceback.format_exc())
...
def adsbProcessing(parentThreadName):
                        ...
                        for formAcft in activeFormationX2: # iterate through each aircraft in the active formation list
                            ...
                            # check callsigns (excluding the last character) to see if they match
                                ...
                                if int(Acft.callSign[-1:]) >=  int(formAcft.callSign[-1:]) - 1 or int(Acft.callSign[-1:]) <=  int(formAcft.callSign[-1:]) + 1:
                                    tempdistance = distance(formAcftPosition.x,formAcftPosition.y,formAcftPosition.z,position.x,position.y,position.z)#distance calculated by Haversine formula
                                    ...
                                    if (tempdistance <= Acft.aircraftType.formationDistThreshold) and Acft.groundSpeed > Acft.aircraftType.fullStopThresholdSpeed and formAcft.groundSpeed > formAcft.aircraftType.fullStopThresholdSpeed:
                                        ...
                                        if (Acft.lastState is None and Acft.takeoffTime is None) or (Acft.lastState is not None and Acft.formTimestamp is not None and formAcft.formTimestamp is not None) and (Acft.lastState == "lost signal" and abs(Acft.formTimestamp - formAcft.formTimestamp) <= timedelta(seconds=Acft.aircraftType.formationLostSignalTimeThreshold)):
                                            if closestFormation is None or tempdistance < closestFormationDistance:
                                                ...
                                                closestFormationDistance = tempdistance 
                        ...
                        for formAcft in activeFormationX4:
                            ...
                            if  Acft.callSign[:-1] == formAcft.callSign[:-1] and Acft.formationX4 is False:
                                if int(Acft.callSign[-1:]) >=  int(formAcft.callSign[-1:]) - 3 or int(Acft.callSign[-1:]) <=  int(formAcft.callSign[-1:]) + 3:
                                    tempdistance = distance(formAcftPosition.x,formAcftPosition.y,formAcftPosition.z,position.x,position.y,position.z)
                                    if (tempdistance <= Acft.aircraftType.formationDistThreshold) and Acft.groundSpeed > Acft.aircraftType.fullStopThresholdSpeed and formAcft.groundSpeed > formAcft.aircraftType.fullStopThresholdSpeed:
                                        if (Acft.lastState is None and Acft.takeoffTime is None) or (Acft.lastState is not None and Acft.formTimestamp is not None and formAcft.formTimestamp is not None) and (Acft.lastState == "lost signal" and abs(Acft.formTimestamp - formAcft.formTimestamp) <= timedelta(seconds=Acft.aircraftType.formationLostSignalTimeThreshold)):
                                            if closestFormation is None or tempdistance < closestFormationDistance:
    ...
    # TODO NEED TO THOROUGHLY TEST THE LOGIC ABOVE AND BELOW THIS LINE. STATE TRANSITIONS ARE CRITICAL TO GET RIGHT. 
        ...
        # print(distance_km)  # Output: 5571.047657186649
                        ...
                        position1 = geometry.Point(0, 0, 0) #initialize position1 and position2 to 0,0,0 
                        position2 = geometry.Point(0, 0, 0)
                        ...
                        for freshAcft in updatedAircraftObjects:        #start form logic 
                            ...
                            if freshAcft.callSign is not None and Acft.callSign is not None and freshAcft.callSign[:-1] == Acft.callSign[:-1]  and not freshAcft.formationX2 and not Acft.formationX2: 
                                formAcftPosition = geometry.Point(0,0,0)
                                position1 = geometry.Point(Acft.latitude, Acft.longitude, int(Acft.alt_baro)) if Acft.alt_baro != "ground" and Acft.latitude is not None else geometry.Point(0, 0, 0)    #find position of 1st jet
                                position2 = geometry.Point(freshAcft.latitude, freshAcft.longitude, int(freshAcft.alt_baro)) if freshAcft.alt_baro  != "ground" and freshAcft.latitude is not None else geometry.Point(0, 0, 0)  #find position of 2nd jet  

                                if (distance(position1.x,position1.y,position1.z,position2.x,position2.y,position2.z) <= Acft.aircraftType.formationDistThreshold) and Acft.groundSpeed > Acft.aircraftType.fullStopThresholdSpeed and freshAcft.groundSpeed > freshAcft.aircraftType.fullStopThresholdSpeed:           # :)  degrees of lat & long to miles Recalc with function above
                            ...
                            # 2 -> 4-ship logic
                                ...
                                position1 = geometry.Point(Acft.latitude, Acft.longitude, int(Acft.alt_baro)) if Acft.latitude is not None else geometry.Point(0, 0)    #find position of 1st jet
                                position2 = geometry.Point(freshAcft.latitude, freshAcft.longitude, int(freshAcft.alt_baro)) if freshAcft.latitude is not None else geometry.Point(0, 0)  #find position of 2nd jet 
                                
                                if (distance(position1.x,position1.y,position1.z,position2.x,position2.y,position2.z) <= Acft.aircraftType.formationDistThreshold) and Acft.groundSpeed > Acft.aircraftType.fullStopThresholdSpeed and freshAcft.groundSpeed > freshAcft.aircraftType.fullStopThresholdSpeed:           # :)  degrees of lat & long to miles
                                    if abs(Acft.timestamp - timezone.now()) <= timedelta(seconds=Acft.aircraftType.formationLostSignalTimeThreshold):
        ...
        logger.error("Error in ADSBProcessingThread Function")
        logger.error(traceback.format_exc())
...
def commsTestThread(parentThreadName, sourceName):
    try:
        ...
        logger.error("Error in Comms Test Thread Function")
        logger.error(traceback.format_exc())
...
def stratuxCommsThread(address, threadName):
    try:
        ...
        logger.error("Error in Stratux Comms Thread Function")
        logger.error(traceback.format_exc())
...
def on_message(ws, message):
    try:
        ...
        logger.error("Error in on_message Function")
        logger.error(traceback.format_exc())
...
def aggregate(reformattedMessage):
    try:
        ...
        logger.error("Error in aggregate Function")
        logger.error(traceback.format_exc())
...
def deleteOldAircraft():
    try:
        ...
        logger.error("Error in deleteOldAircraft Function")
        logger.error(traceback.format_exc())
...
def Stratux_to_ADSBExchangeFormat(inputMessage):
    try:
        ...
        logger.error("Error in Stratux_to_ADSBExchangeFormat Function")
        logger.error(traceback.format_exc())
...
def adsbExchangeCommsThread(parentThreadName, lat, lon, radius):
    try:
        ...
        logger.error("Error in ADSBExchangeCommsThread Function")
        logger.error(traceback.format_exc())
...
def inPattern(position, patternDict):
    try:
        ...
        logger.error("Error in inPattern Function")
        logger.error(traceback.format_exc())
...
def getKMLplacemark(file, placemarkName):
    try:
        ...
        logger.error("Error in getKMLplacemark Function")
        logger.error(traceback.format_exc())
...
def swapLatLong(polygon):
    try:
        ...
        logger.error("Error in swapLatLong Function")
        logger.error(traceback.format_exc())
...
def parse_placemarks(target, document):
    try:
        ...
        logger.error("Error in parse_placemarks Function")
        logger.error(traceback.format_exc())
...
def getAircraftNotUpdated(updatedAircraftList):
    try:
        ...
        logger.error("Error in getAircraftNotUpdated Function")
        logger.error(traceback.format_exc())
...
def getPosition(aircraft):
    try:
        ...
        logger.error("Error in getPosition Function")
        logger.error(traceback.format_exc())
...
def setSubstate(position, state, patternDict):
    try:
        ...
        logger.error("Error in setSubstate Function")
        logger.error(traceback.format_exc())
...
def resetNextTakeoffData(nextTOData):
    try:
        ...
        logger.error("Error in resetNextTakeoffData Function")
        logger.error(traceback.format_exc())
...
def commsManagementThread(parentThreadName):
    ...
    try:
        ...
        logger.error("Error in CommsManagementThread Function")
        logger.error(traceback.format_exc())
...
def adsbSource(parentThreadName, source): #takes new source of ADSB data that was created and put it info on a new thread, make connection, process data, get permission to edit mutex json and insert data.
    try:
        ...
        logger.error("Error in adsbSource Function")
        logger.error(traceback.format_exc())
